...
const QuotesList: React.FC<QuotesListProps> = ({ onQuoteSelect, compact = false }) => {
  ...
  useEffect(() => {
    ...
    const handleOutside = (e: MouseEvent) => {
      ...
      if (target && target.closest('[data-status-menu]')) return;
  ...
  const handleStatusChange = async (quote: Quote, newStatus: QuoteStatus) => {
    setOpenStatusId(null);
    if (getQuoteStatus(quote) === newStatus) return;
    try {
      setStatusBusyId(quote.id!);
      await quotesAPI.update(quote.id!, { quote_status: newStatus });
      setQuotes((prev) =>
        prev.map((q) => (q.id === quote.id ? { ...q, quote_status: newStatus } : q))
      );
    } catch (err) {
              ...
              {days.map((dayKey) => (
                  <div key={dayKey} className="card rounded-xl bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm">
                    ...
                    <div className="space-y-3">
                      {groupedQuotes[dayKey].map((quote, index) => (
                          <div
                              key={quote.id}
                              className={`border border-gray-200/90 dark:border-gray-700 rounded-2xl p-3 sm:p-4 backdrop-blur-sm shadow-[0_1px_2px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_rgba(15,23,42,0.12)] dark:hover:shadow-[0_8px_24px_rgba(0,0,0,0.35)] transition-all duration-200 ${
                                index % 2 === 0
                                  ? 'bg-white dark:bg-gray-800'
                                  : 'bg-slate-100 dark:bg-gray-900/70'
                              }`}
                          >
                              ...
                              >
                                <div className="flex flex-wrap items-center gap-2 mb-1.5">
                                  ...
                                  <div className="relative" data-status-menu>
                                    <button
                                      type="button"
                                      disabled={statusBusyId === quote.id}
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setOpenStatusId((prev) => (prev === quote.id ? null : quote.id!));
                                      }}
                                      className={`inline-flex items-center gap-1 px-2.5 py-1 text-[11px] rounded-full font-medium transition-opacity hover:opacity-80 disabled:opacity-50 disabled:pointer-events-none ${STATUS_BADGE_CLASSES[getQuoteStatus(quote)]}`}
                                      title="שינוי סטטוס"
                                    ...
                                    >
                                      ...
                                      <svg
                                        className={`w-3 h-3 transition-transform ${openStatusId === quote.id ? 'rotate-180' : ''}`}
                                        viewBox="0 0 20 20"
                                        fill="currentColor"
                                        aria-hidden="true"
                                      >
                                    ...
                                    {openStatusId === quote.id && (
                                      <div
                                        ...
                                        className="absolute z-30 mt-1 right-0 min-w-[11rem] rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg py-1"
                                      >
                                        {(Object.keys(STATUS_LABELS) as QuoteStatus[]).map((statusKey) => {
                                          const isCurrent = getQuoteStatus(quote) === statusKey;
                                          return (
                                            <button
                                              key={statusKey}
                                              type="button"
                                              ...
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                void handleStatusChange(quote, statusKey);
                                              }}
                                              className={`w-full flex items-center justify-between gap-2 px-3 py-2 text-right text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${isCurrent ? 'font-semibold text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-200'}`}
                                            >
                                              <span className="inline-flex items-center gap-2">
                                                <span className={`w-2.5 h-2.5 rounded-full ${STATUS_BADGE_CLASSES[statusKey]}`} />
